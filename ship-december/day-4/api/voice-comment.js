// Voice Comment API handler
// Accepts audio, transcribes with OpenAI, saves audio blob + metadata to voice-comments.md
...
async function saveBlob(filePath, base64Content, message, githubToken) {
   ...
   const headers = {
      ...
      'User-Agent': 'ship-december-voice-comments'
   };
...
export async function onRequest(context) {
   ...
   try {
      ...
      const blobFilename = `${timestamp}-${safeName}.webm`;
      ...
      await saveBlob(
         blobPath,
         audio,
         `Add voice comment audio from ${userName}`,
         githubToken
      );
...
**Transcript:** ${transcript}`;
      ...
      // Append to voice-comments.md
      const filePath = `ship-december/${dayStr}/voice-comments.md`;
      const result = await appendToFile(
         filePath,
         voiceComment,
         `Add voice comment from ${userName} to ${dayStr}`,
         githubToken
      );
